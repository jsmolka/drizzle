  ...
  static constexpr std::string_view repr(const Expression::Binary::Type& value) {
    ...
    switch (value) {
      case Expression::Binary::Type::Addition:
        return "+";
      case Expression::Binary::Type::And:
        return "&&";
      ...
      case Expression::Binary::Type::NotEqual:
        return "!=";
      case Expression::Binary::Type::Or:
        return "||";
  ...
  auto format(const Expression::Literal& value, FormatContext& ctx) {
    ...
    switch (value.type) {
      case Expression::Literal::Type::Boolean:
        return fmt::format_to(ctx.out(), "{}", std::get<dzbool>(value.value));
      case Expression::Literal::Type::Float:
        return fmt::format_to(ctx.out(), "{}", std::get<dzfloat>(value.value));
      case Expression::Literal::Type::Integer:
        return fmt::format_to(ctx.out(), "{}", std::get<dzint>(value.value));
      case Expression::Literal::Type::Null:
        return fmt::format_to(ctx.out(), "null");
      case Expression::Literal::Type::String:
        return fmt::format_to(ctx.out(), "\"{}\"", std::get<std::string>(value.value));
  ...
  auto format(const DzValue& value, FormatContext& ctx) {
    ...
    switch (value.type) {
      case DzValue::Type::Bool:
        return fmt::format_to(ctx.out(), "{}", value.b);
      case DzValue::Type::Int:
        return fmt::format_to(ctx.out(), "{}", value.i);
      case DzValue::Type::Float:
        return fmt::format_to(ctx.out(), fmt::runtime(is_whole(value.f) ? "{:.1f}" : "{}"),
                              value.f);
      case DzValue::Type::Object:
        return fmt::format_to(ctx.out(), "{}", *value.o);
