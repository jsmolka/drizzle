...
struct fmt::formatter<Expression::Type> : fmt::formatter<std::string_view> {
  static constexpr auto repr(const Expression::Type& value) -> std::string_view {
    ...
    switch (value) {
      ...
      default: SH_UNREACHABLE; return "unreachable";
    }
  }
  ...
  auto format(const Expression::Literal& value, FormatContext& ctx) {
    ...
    switch (value.type) {
      ...
      case Expression::Literal::Type::String:
        return fmt::format_to(ctx.out(), R"("{}")", std::get<std::string>(value.value));
      default: SH_UNREACHABLE; return fmt::format_to(ctx.out(), "unreachable");
    }
  }
};
...
struct fmt::formatter<DzValue> {
  ...
  auto format(const DzValue& value, FormatContext& ctx) {
    auto whole = [](double v) { return std::fmod(v, 1.0) == 0.0; };
    ...
    switch (value.type) {
      ...
      case DzValue::Type::Float:
        return fmt::format_to(ctx.out(), fmt::runtime(whole(value.f) ? "{:.1f}" : "{}"), value.f);
