...
const Parser::Rule& Parser::rule(Token::Type type) {
  ...
  static constexpr auto kRules =
      sh::make_array<Parser::Rule, kRulesSize>([](auto type) -> Parser::Rule {
        switch (Token::Type(type)) {
          case Token::Type::And:
            return {nullptr, &Parser::binary, Precedence::BitAnd};
          case Token::Type::And2:
            return {nullptr, &Parser::and_, Precedence::And};
          case Token::Type::Bang:
            return {&Parser::unary, nullptr, Precedence::Term};
          case Token::Type::BangEqual:
            return {nullptr, &Parser::binary, Precedence::Equality};
          case Token::Type::Caret:
            return {nullptr, &Parser::binary, Precedence::BitXor};
          case Token::Type::Equal2:
            return {nullptr, &Parser::binary, Precedence::Equality};
          case Token::Type::False:
            return {&Parser::literal, nullptr, Precedence::None};
          case Token::Type::Float:
            return {&Parser::constant, nullptr, Precedence::Term};
          case Token::Type::Greater:
            return {nullptr, &Parser::binary, Precedence::Comparison};
          case Token::Type::Greater2:
            return {nullptr, &Parser::binary, Precedence::BitShift};
          case Token::Type::Greater3:
            return {nullptr, &Parser::binary, Precedence::BitShift};
          case Token::Type::GreaterEqual:
            return {nullptr, &Parser::binary, Precedence::Comparison};
          case Token::Type::Identifier:
            return {&Parser::variable, nullptr, Precedence::None};
          ...
          case Token::Type::ParenLeft:
            return {&Parser::group, nullptr, Precedence::None};
          case Token::Type::Percent:
            return {nullptr, &Parser::binary, Precedence::Factor};
          case Token::Type::Pipe:
            return {nullptr, &Parser::binary, Precedence::BitOr};
          case Token::Type::Pipe2:
            return {nullptr, &Parser::or_, Precedence::Or};
          ...
          case Token::Type::String:
            return {&Parser::constant, nullptr, Precedence::None};
          case Token::Type::Tilde:
            return {&Parser::unary, nullptr, Precedence::Unary};
...
void Parser::expect(Token::Type type, std::string_view error) {
  if (!match(type)) throw SyntaxError(current->lexeme.data(), error);
}
...
void Parser::parseExpression(Precedence precedence) {
  ...
  if (!prefix) throw SyntaxError(previous->lexeme.data(), "invalid syntax");
  ...
  while (precedence <= rule(current->type).precedence) {
    ...
    if (!infix) throw SyntaxError(previous->lexeme.data(), "invalid syntax");
...
Stmt Parser::declaration() {
  if (match(Token::Type::Var)) return declarationVar();
...
Stmt Parser::statement() {
  if (match(Token::Type::Block)) return statementBlock();
  if (match(Token::Type::Break)) return statementBreak();
  if (match(Token::Type::Continue)) return statementContinue();
  if (match(Token::Type::If)) return statementIf();
  if (match(Token::Type::Noop)) return statementNoop();
  if (match(Token::Type::Print)) return statementPrint();
  if (match(Token::Type::While)) return statementWhile();
...
Stmt Parser::statementBlock() {
  ...
  if (match(Token::Type::Identifier)) identifier = previous->lexeme;
